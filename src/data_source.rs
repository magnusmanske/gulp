...
use crate::row::Row;
use crate::{app_state::AppState, header::*, GulpError};
...
impl DataSourceFormatJSONL {
    pub fn guess_headers(&self, lines: &[String]) -> Vec<HeaderColumn> {
        let columns = lines
            .iter()
...
impl DataSourceFormatPagePile {
    pub fn get_lines_actually(
        &self,
        header_file: &mut FileWithHeader,
        limit: usize,
    ) -> Result<(String, Vec<String>), GulpError> {
        ...
        let wiki = json
            .get("wiki")
            .ok_or("import_from_pagepile: No field 'wiki'")?
            .as_str()
            .ok_or("import_from_pagepile: field 'wiki' not a str")?
            .to_string();
        let iterator = json
            .get("pages")
            .ok_or("import_from_pagepile: No field 'pages'")?
            .as_object()
            .ok_or("import_from_pagepile: field 'pages' not an object")?
            .keys()
            .take(limit)
            .cloned();
...
impl DataSourceFormat {
    ...
    pub fn line_converter(&self) -> Box<dyn DataSourceLineConverter> {
        match self {
...
impl fmt::Display for DataSourceFormat {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            Self::CSV => write!(f, "CSV"),
            Self::TSV => write!(f, "TSV"),
            Self::JSONL => write!(f, "JSONL"),
            Self::PAGEPILE => write!(f, "PAGEPILE"),
            Self::EXCEL => write!(f, "XLS"),
        }
    }
}
...
impl DataSourceType {
    ...
    pub fn line_handler(&self) -> Box<dyn DataSourceAsFile> {
        match self {
...
impl DataSource {
    ...
    pub async fn from_db(app: &Arc<AppState>, source_id: DbId) -> Option<Self> {
        ...
        app.get_gulp_conn()
            .await
            .ok()?
            .exec_iter(sql, params! {source_id})
            .await
            .ok()?
    ...
    pub async fn create(&mut self, app: &Arc<AppState>) -> Option<DbId> {
        ...
        conn.exec_drop(
            sql,
            params! {list_id,source_type,source_format,location,user_id},
        )
        .await
        .ok()?;
    ...
    pub async fn guess_headers(&self, limit: Option<usize>) -> Result<CellSet, GulpError> {
        ...
        let cell_set = self
            .source_format
            .line_converter()
            .get_cells(&mut header_file, limit)?;
