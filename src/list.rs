...
use crate::cell::*;
use crate::data_source::{CellSet, DataSource};
use crate::header::*;
use crate::row::*;
use crate::GulpError;
...
impl List {
    ...
    pub async fn set_header_schema(
        &mut self,
    ...
    pub async fn add_access(
        ...
        app.get_gulp_conn()
            .await?
            .exec_drop(sql, params! {list_id,user_id,access})
            .await?;
        Ok(())
    }
    ...
    pub async fn get_rows_for_revision(&self, revision_id: DbId) -> Result<Vec<Row>, GulpError> {
        self.get_rows_for_revision_paginated(revision_id, 0, None)
            .await
    }

    pub async fn get_rows_for_revision_paginated(
        &self,
        revision_id: DbId,
        start: DbId,
        length: Option<DbId>,
    ) -> Result<Vec<Row>, GulpError> {
        ...
        let row_opts = self
            ...
            .get_gulp_conn()
            .await?
            .exec_iter(sql, params! {list_id,revision_id,start,length})
            .await?
            .map_and_drop(|row| Row::from_row(&row, &self.header))
            .await?;
        let rows: Vec<Row> = row_opts.iter().flatten().cloned().collect();
        Ok(rows)
    }

    pub async fn get_users_in_revision(
        &self,
        revision_id: DbId,
    ) -> Result<HashMap<DbId, String>, GulpError> {
        ...
        let ret = self
            ...
            .await?
            .map_and_drop(mysql_async::from_row::<(DbId, String)>)
            .await?
            .into_iter()
            .collect();
        Ok(ret)
    ...
    pub fn get_client() -> Result<reqwest::Client, GulpError> {
        let client = reqwest::Client::builder().user_agent("gulp/0.1").build()?;
        Ok(client)
    }
    ...
    async fn load_json_md5s(&self, conn: &mut Conn) -> Result<HashSet<String>, GulpError> {
        ...
        let ret = conn
            .exec_iter(sql, params! {list_id})
            .await?
            .map_and_drop(mysql_async::from_row::<String>)
            .await?;
        let ret: HashSet<String> = ret.into_iter().collect();
        Ok(ret)
    }

    async fn flush_row_insert(
        &self,
        conn: &mut Conn,
        rows: &mut Vec<Row>,
    ) -> Result<(), GulpError> {
        ...
        let params: Vec<mysql_async::Params> = rows
            .iter()
            .map(|row| {
    ...
    pub async fn get_sources(&self) -> Result<Vec<DataSource>, GulpError> {
        ...
        let sources = self
            ...
            .exec_iter(sql, params! {list_id})
            .await?
            .map_and_drop(|row| DataSource::from_row(&row))
            .await?
            .iter()
            .flatten()
            .cloned()
    ...
    pub async fn snapshot(&mut self) -> Result<DbId, GulpError> {
        ...
        let results = conn
            ...
            .await?;
        let numer_of_rows_with_current_revision = results.first();
        ...
        if numer_of_rows_with_current_revision == 0 {
            // No need to make a new snapshot
    ...
    async fn check_json_exists(
        &self,
        _conn: &mut Conn,
        _json_text: &str,
        _json_md5: &str,
    ) -> Result<bool, GulpError> {
        // Already checked via md5, might have to implement if collisions occur
        Ok(true)
    }

    async fn get_or_ignore_new_row(
        &self,
        conn: &mut Conn,
        md5s: &HashSet<String>,
        ...
        row_num: DbId,
        user_id: DbId,
    ) -> Result<Option<Row>, GulpError> {
        let cells2j: Vec<_> = cells
            .iter()
            .zip(self.header.schema.columns.iter())
            .collect();
        let cells2j: Vec<serde_json::Value> = cells2j
            .iter()
            .cloned()
        ...
        let json_exists = if md5s.contains(&json_md5) {
            self.check_json_exists(conn, &cells_json_text, &json_md5)
                .await?
        } else {
            false
        };
