...
pub trait DataSourceLineConverter {
    fn get_cells(&self, header_file: &mut FileWithHeader, limit: Option<usize>) -> Result<CellSet,GulpError> ;

    fn get_lines(&self, header_file: &mut FileWithHeader, limit: usize) -> Result<LinesReturnType,GulpError> {
    ...
    fn get_cells_xsv(&self, separator: u8, header_file: &mut FileWithHeader, limit: Option<usize>) -> Result<CellSet,GulpError> {
...
impl DataSourceLineConverter for DataSourceFormatExcel {
    fn get_cells(&self, header_file: &mut FileWithHeader, limit: Option<usize>) -> Result<CellSet,GulpError> {
        let limit = limit.unwrap_or(usize::MAX);
        let mut reader = Arc::get_mut(&mut header_file.file).ok_or_else(||"Cannot get temporaryfile handle")?;
        ...
        let first_workbook_name = workbook.sheet_names().map_err(|e|e.to_string())?
            .get(0).ok_or_else(||"No worksheets in Excel file")?
            .to_owned();
        let range = workbook.worksheet_range(&first_workbook_name).map_err(|e|e.to_string())?;
