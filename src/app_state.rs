...
use crate::GulpError;
use crate::{header::DbId, list::List};
...
impl AppState {
    ...
    pub fn get_api_for_wiki(wiki: &str) -> Result<wikibase::mediawiki::api::Api, GulpError> {
        ...
        let api = std::thread::spawn(move || {
            tokio::runtime::Runtime::new()
                .unwrap()
                .block_on(async { wikibase::mediawiki::api::Api::new(&api_url).await })
        })
        .join()
    ...
    pub async fn get_lists_by_user_rights(
        &self,
        user_id: DbId,
        rights: &str,
    ) -> Option<Vec<Value>> {
        ...
        let sql = if rights.is_empty() {
            r#"SELECT list.id,list.name,list.revision_id,GROUP_CONCAT(DISTINCT `right`) FROM `list`,`access` WHERE user_id=:user_id AND list_id=list.id GROUP BY list.id"#.to_string()
        } else {
            let rights = format!("\"{}\"", rights.join("\",\""));
            format!(
                r#"SELECT list.id,list.name,list.revision_id,GROUP_CONCAT(DISTINCT `right`) FROM `list`,`access` WHERE user_id=:user_id AND list_id=list.id AND `right` IN ({rights}) GROUP BY list.id"#
            )
        };
        ...
        let rows = self
            ...
            .await
            .ok()?
            .map_and_drop(mysql_async::from_row::<(DbId, String, DbId, String)>)
            .await
            .ok()?;
    ...
    pub async fn get_list(self: &Arc<AppState>, list_id: DbId) -> Option<Arc<Mutex<List>>> {
        if !self.lists.read().await.contains_key(&list_id) {
            ...
            self.lists
                ...
                .await
                .entry(list_id)
                .or_insert(Arc::new(Mutex::new(list)));
        }
    ...
    pub fn get_server_for_wiki(wiki: &str) -> String {
        lazy_static! {
            static ref RE_REMOVE_P: Regex =
                Regex::new(r#"_p$"#).expect("RE_REMOVE_P does not parse");
            static ref RE_FINAL_WIKI: Regex =
                Regex::new(r#"wiki$"#).expect("RE_FINAL_WIKI does not parse");
            static ref RE_OTHER: Regex =
                Regex::new(r#"^(.+)(wik.+)$"#).expect("RE_FINAL_WIKI does not parse");
        }
        ...
        match wiki.as_str() {
            ...
            wiki => {
                ...
                if server == wiki {
                    RE_OTHER
                        .replace(&wiki, |caps: &Captures| {
                            format!("{}.{}.org", &caps[1], &caps[2])
                        })
                        .to_string()
                } else {
                    server
    ...
    pub async fn get_all_header_schemas(
        &self,
    ) -> Result<Vec<crate::header::HeaderSchema>, GulpError> {
        ...
        Ok(self
            ...
            .exec_iter(sql, ())
            .await?
            .map_and_drop(|row| crate::header::HeaderSchema::from_row(&row))
            .await?
            .iter()
...
mod tests {
    ...
    fn test_get_server_for_wiki() {
        ...
        assert_eq!(
            AppState::get_server_for_wiki("enwiki_p"),
            "en.wikipedia.org"
        );
        assert_eq!(
            AppState::get_server_for_wiki("commonswiki"),
            "commons.wikimedia.org"
        );
        assert_eq!(
            AppState::get_server_for_wiki("wikidatawiki"),
            "www.wikidata.org"
        );
        assert_eq!(
            AppState::get_server_for_wiki("specieswiki"),
            "species.wikimedia.org"
        );
        assert_eq!(
            AppState::get_server_for_wiki("metawiki"),
            "meta.wikimedia.org"
        );
    }
